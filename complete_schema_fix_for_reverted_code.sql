...
-- This script creates missing tables and fixes existing ones to match the "Refresh button and 2am refresh" commit
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'journal_entries' AND table_schema = 'public') THEN
        ...
        -- Add constraint for entry_type values if it doesn't exist
...
-- Handle foreign key constraints by updating existing journal entries first
-- Set guided_question_id to NULL for entries that reference non-existent questions
UPDATE public.journal_entries 
...
-- Now we can safely clear existing guided questions
